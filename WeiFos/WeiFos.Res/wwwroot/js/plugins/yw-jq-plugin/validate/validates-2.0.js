...
var validateType = {
    ...
    isPhone: function (phone) {
        return /(^[0-9]{3,4}\-[0-9]{7,8}$)|(^[0-9]{7,8}$)|(^\([0-9]{3,4}\)[0-9]{3,8}$)|(^0{0,1}1[345678]\d{9}$)/.test(phone);
    },
    /// <summary>
    /// 邮编格式验证  
    /// @param postCode 
    ...
    yw.valid.validate = function (obj, options) {
        ...
        if (options.vtype($.trim(obj.val()))) {
            ...
            if (!$.trim(obj.val()).length > 0) {
                ...
                return yw.valid.msg.error(obj, { errormsg: nullmsg, afterelementId: options.afterelementId });
            }
