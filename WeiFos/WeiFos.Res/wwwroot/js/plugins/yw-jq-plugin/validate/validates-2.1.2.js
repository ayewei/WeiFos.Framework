...
yw.valid.getValidate = function (options) {
    ...
    $.each(opt.submitelements, function (i, o) {
        if (App_G.Util.isJQuery(options.submitelements)) {
            o = o.selector;
        }
            ...
            $.each($this, function (i, o) {
                ...
                switch ($(o).prop("type")) {
                    ...
                    case "text":
                        ...
                        //该元素得到焦点事件
                        current.on("focus", "#" + $(o).attr("id"), function (e) {
                            yw.valid.msg.focus($("#" + $(o).attr("id")), _options);
                        });

                        //失去焦点事件
                        current.on("blur", "#" + $(o).attr("id"), function (e) { 
                            yw.valid.validate($($("#" + $(o).attr("id"))), _options);
                        });
                    ...
                    case "select-one":
                        //选择改变事件 
                        current.on("change", "#" + $(o).attr("id"), function (e) { 
                            yw.valid.validate($("#" + $(o).attr("id")), _options);
                        }); 
                        break;
                        //复选框
                    case "checkbox":
                        //单击事件
                        current.on("click", "#" + $(o).attr("id"), function (e) {
                            var obj = $("#" + $(o).attr("id"));
                            if (obj.is(':checked')) {
    ...
    yw.valid.validate = function (obj, options) {
        ...
        if (options.vtype($.trim(obj.val()))) {
            ...
            if (!$.trim(obj.val()).length > 0) {
                var nullmsg = (undefined == options.nullmsg) ? "不能为空" : options.nullmsg;
                return yw.valid.msg.error(obj, { errormsg: nullmsg, afterelementId: options.afterelementId, model: options.model });
            }
