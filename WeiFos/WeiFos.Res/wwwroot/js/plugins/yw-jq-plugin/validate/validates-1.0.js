...
var validateType = {
    ...
    isEnglishName: function (englishName) {
        return /^[a-zA-Z. ]{1,100}$/.test(englishName);
    },
    /// <summary>
    /// 货币英文简写验证
    /// @param abbreviation
    ...
    /// @returns boolean
    /// </summary>s
    isCurrencyCode: function (currencyCode) {
        return /^[0-9]{3}$/.test(currencyCode);
    },
    /// <summary>
    /// 补减价格验证
    /// @param price
    ...
    isCompanyName: function (companyName) {
        return /^[a-zA-Z0-9-_\u4e00-\u9fa5]{1,40}$/.test(companyName);
    ...
    /// @returns boolean
    /// </summary>
    isOtherPhone: function (phone) {
        return /^[0-9\-()（）]{7,18}$/.test(phone);
    },
    /// <summary>
    /// 邮编格式验证  
    /// @param postCode 
    ...
    /// @returns boolean
    /// </summary>
    isNumberlatter: function (userName) {
        return /(?!^\d+$)(?!^[a-zA-Z]+$)[0-9a-zA-Z]{4,23}/.test(userName);
    },

    isChinese: function (userName) {
        return /[\u4e00-\u9fa5]{2,4}/.test(userName);
    },
    ...
    isDocUrl: function (url) {
        return /^([A-Za-z]{1}:\/[\w\/]*)?\w+\.{1}[a-zA-Z]+$/i.test(url);
    },
    ...
    isLGZeroNumber: function (number) {
        return /^([1-9]\d{0,3}|1000000)$/g.test(number);
    },
...
(function ($) {
    ...
    var _validates = function () {
        ...
        $.each(validateElement.elementItems, function (i, o) {
            if (!_validate($("#" + o.optionsId), o) && ispass) {
