...
$(function () {
    ...
    $("#ad_img").upload({
        ...
        imgurl: $("#imgurl").val(),
        cData: {
            bizType: App_G.ImgType.Case_Cover,
    ...
    editor.ready(function () {
        ...
        editor.execCommand('serverparam', {
            'bizType': App_G.ImgType.Case_Details,
            'bizId': $("#Ticket").val(),
    ...
    //详细
    var wx_editor = UE.getEditor('wechat_context');
    wx_editor.ready(function () {
        //自定义参数
        wx_editor.execCommand('serverparam', {
            'bizType': App_G.ImgType.Case_Details,
            'bizId': $("#Ticket").val(),
            'createThmImg': App_G.CreateThmImg.None
        });
        ...
        submitelements: "#SaveBtn", vsuccess: function () {
            ...
            var data = {
                entity: App_G.Mapping.Get("data-val", {
                    ...
                    wechat_context: wx_editor.getContent()
                }),
                imgmsg: img.data == undefined ? "" : img.data
            };

            $post("/SiteSetting/SaveCase", JSON.stringify(data),
                 function (result) {
                     if (result.Code == App_G.Code.Code_200) {
                         App_G.MsgBox.success_digbox();
                         setTimeout("window.location.href='CaseManage';", 1000);
                     } else {
    ...
    v.valid("#title", { focusmsg: "请输入案例标题", errormsg: "", vtype: verifyType.anyCharacter });
