...
$(function () {
    ...
    yw.valid.config({
        submitelements: "#SaveBtn", vsuccess: function () {
            $("#SaveBtn").setDisable();
            var data = {
                informtCgty: App_G.Mapping.Get("data-val", { id: App_G.Util.getRequestId('bid') }),
                imgmsg: img.data == undefined ? "" : img.data
            };

            $postByAsync("/SiteSetting/SaveInformtCgty", JSON.stringify(data),
                 function (data) {
                     if (data.Code == App_G.Code.Code_200) {
                         App_G.MsgBox.success_digbox();
                         setTimeout("window.location.href='InformtCgtyManage';", 1000);
                     } else {
    ...
    v.valid("#name", { focusmsg: "请输入分类名称", errormsg: "名称长度在1~15字符", vtype: verifyType.anyCharacter });
    v.valid("#prefix_no", { focusmsg: "请输入前缀编号", errormsg: "字母数字或下划线", vtype: verifyType.isSerialNumber });
    v.valid("#order_index", { focusmsg: "请输入排序索引", errormsg: "索引只能够是正整数", vtype: verifyType.isLGZeroNumber });
    v.valid("#introduction", { focusmsg: "请输入分类简介", errormsg: "", othermsg: "", vtype: verifyType.anyCharacter, selectvalidate: true });
