...
$(function () {
    ...
    datagrid = $("[name=datagrid]").datagrid({
        url: "/OrgModule/Org/GetDepartments",
        ...
        dblclick: function (tr) {
            window.location.href = "/OrgModule/Org/DepartmentForm?bid=" + tr.attr("data-id");
        }
    });
    ...
    $("[name=add_btn]").click(function () {
        window.location.href = "/OrgModule/Org/DepartmentForm";
    });
    ...
    $("[name=delete_btn]").digbox({
        ...
        CallBack: function (s, c, p) {
            $post("/OrgModule/Org/DelDepartment?bid=" + datagrid.getSeleteTr().attr("data-id"), "",
                function (result) {
    ...
    if (data != null && data.length > 0) {
        ...
        $.each(data, function (i, o) {
            ...
            if (o.parent_id == id) {
                ...
                //获取子节点
                var cnode = GetChilds(o.id, o.childs);
                if (cnode.length > 0) node.nodes.push(cnode);
                nodes.push(node);
            }
        });
    }
