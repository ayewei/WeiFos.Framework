...
$(function () {
    ...
    datagrid = $("[name=datagrid]").datagrid({
        ...
        dblclick: function (tr) {
            window.location.href = "/ProductModule/Product/GuidePdtCatgForm?bid=" + tr.attr("data-id");
        },
    ...
    $("[name=add_btn]").click(function () {
        window.location.href = "/ProductModule/Product/GuidePdtCatgForm";
    });
    ...
    $("[name=edit_btn]").click(function () {
        ...
        window.location.href = '/ProductModule/Product/GuidePdtCatgForm?bid=' + datagrid.getSeleteTr().attr("data-id");
    });
    ...
    $("[name=delete_btn]").digbox({
        ...
        CallBack: function (s, c, p) {
            $get("/ProductModule/Product/DeletePdtGuideCatg?bid=" + datagrid.getSeleteTr().attr("data-id"), "",
                function (result) {
                    if (result.Code == App_G.Code.Code_200) {
    ...
    $("[name=save_orderindex]").click(function () {
        ...
        if (parmas.length > 0) {
            $post("/ProductModule/Product/SaveGuideCatgOrderIndex", JSON.stringify({ data: parmas }),
                function (result) {
                    if (result.Code == App_G.Code.Code_200) {
                        layer.msg(result.Message, { icon: 1 });
                        datagrid.execute(getSearchData());
