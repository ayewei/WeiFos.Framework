...
$(function () {
    ...
    datagrid = $("[name=datagrid]").datagrid({
        url: "/ProductModule/Product/GetBrands",
        data: getSearchData(),
        pager: {
            index: 0,
            pageSize: [10, 20, 50]
    ...
    $("[name=edit_btn]").click(function () {
        ...
        window.location.href = '/ProductModule/Product/BrandForm?bid=' + datagrid.getSeleteTr().attr("data-id");
    });
    ...
    $("[name=delete_btn]").digbox({
        ...
        CallBack: function (s, c, p) {
            $post("/ProductModule/Product/DeleteBrand?bid=" + datagrid.getSeleteTr().attr("data-id"), "",
                function (result) {
