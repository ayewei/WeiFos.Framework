...
$(function () {
    ...
    datagrid = $("[name=datagrid]").datagrid({
        url: "/SKUModule/SKU/GetSpecNames?bid=" + App_G.Util.getRequestId('bid'),
        data: getSearchData(),
    ...
    $("[name=delete_btn]").digbox({
        ...
        CallBack: function (s, c, p) {
            $post("/SKUModule/SKU/DeleteSpecName?bid=" + datagrid.getSeleteTr().attr("data-id"), "",
                function (result) {
                    if (result.Code == App_G.Code.Code_200) {
    ...
    $("a[name=delete_btn_n]").digbox({
        ...
        CallBack: function (b, c, p) {
            ...
            $get("/SKUModule/SKU/DeleteSpecValue?bid=" + li.attr("data-id"), "",
                function (result) {
                    if (result.Code == App_G.Code.Code_200) {
    ...
    $("#NextBtn").click(function () {
        window.location.href = "/SKUModule/SKU/ProductTypeManage?bid=" + App_G.Util.getRequestId('bid');
    });
...
function submit(btn, p, c, type) {
    ...
    v = yw.valid.getValidate({
        vsuccess: function () {
            $post("/SKUModule/SKU/SaveSpecName", JSON.stringify(GetData()),
                function (result) {
                    if (result.Code == App_G.Code.Code_200) {
                        layer.msg(result.Message, { icon: 1 });
                        datagrid.execute(getSearchData());
...
function submit_val(c, p) {
    ...
    v1 = yw.valid.getValidate({
        ...
        vsuccess: function () {
            $post("/SKUModule/SKU/SaveSpecValue", JSON.stringify(GetValData(c)),
                function (result) {
                    if (result.Code == App_G.Code.Code_200) {
                        layer.msg(result.Message, { icon: 1 });
                        datagrid.execute(getSearchData());
...
function GetData() {
    ...
    return {
        specName: App_G.Mapping.Get("data-val", { id: id, product_type_id: App_G.Util.getRequestId('bid') }, diglog),
        specValues: diglog.find("#spec_values").val()
    };
}
...
function GetValData(c) {
    ...
    return {
        specValue: {
            ...
            specname_id: c.parent().parent().attr("data-spid"), val: diglog.find("#val").val()
        }
    };
}
...
//扩展属性名称模板
var tmp = '<div style="height:140px;width:640px;"  >\
                     ...
                     <tr>\
                     <th scope="row">规格值：</th>\
                     <td><input class="form-control form-control-sm"  type="text" id="spec_values"  maxlength="100" style="width:300px;"  /><br>扩展属性的值，多个属性值可用“,”号隔开，每个值的字符数最多15个字符\
                     </td>\
                     </tr></table>\
                     </div>';
...
//属性名称修改时 使用
var tmp_u = '<div style="height:80px;width:640px;" >\
             <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_s1">\
             <tr>\
             <th scope=row>规格名称：</th>\
             <td> <input class="form-control form-control-sm"  type=text id="name" data-val="name" maxlength="30" /> </td>\
