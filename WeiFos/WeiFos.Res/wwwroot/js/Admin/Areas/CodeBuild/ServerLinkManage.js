...
$(function () {
    ...
    datagrid = $("[name=datagrid]").datagrid({
        url: "/CodeBuildModule/CodeBuild/GetServerLinks",
        data: getSearchData(),
        pager: {
            index: 0,
            pageSize: [10, 20, 50]
        ...
        dblclick: function (tr) {
            window.location.href = '/CodeBuildModule/CodeBuild/ServerLinkForm?bid=' + tr.attr("data-id");
        },
    ...
    $("[name=add_btn]").click(function () {
        window.location.href = '/CodeBuildModule/CodeBuild/ServerLinkForm';
    });
    ...
    $("[name=edit_btn]").click(function () {
        ...
        window.location.href = '/CodeBuildModule/CodeBuild/ServerLinkForm?bid=' + datagrid.getSeleteTr().attr("data-id");
    });
    ...
    $("div.dataTables_filter").digbox({
        ...
        CallBack: function (s, c, p) {
            $get("/CodeBuildModule/CodeBuild/DelServerLink?bid=" + datagrid.getSeleteTr().attr("data-id"), "",
                function (result) {
                    if (result.Code == App_G.Code.Code_200) {
