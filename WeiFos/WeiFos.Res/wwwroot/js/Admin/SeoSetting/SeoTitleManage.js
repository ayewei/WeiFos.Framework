...
function getSearchData() {
    return { name: $("#Name").val(), ctgy_id: $("#CgtyID").val(), createdDate: $.trim($("#CreatedDate").val()), type: 0 };
}
...
$(function () {
    ...
    var pager = $("#PagerDiv").pager({
        url: '/SeoSetting/GetSeoKeyWords?rnd=' + Math.random(),
        data: getSearchData(),
        currentPage: 0,
        pageSize: 15,
    ...
    $("[name=delete_btn]").digbox({
        ...
        CallBack: function (s, c, p) {
            ...
            $get("/SeoSetting/DeleteSeoKeyWord?bid=" + c.parent().attr("data-id"), "",
              function (data) {
    ...
    $("[name=enable]").digbox({
        ...
        CallBack: function (s, c, p) {
            ...
            $get("/SeoSetting/SetSeoKeyWordEnable?bid=" + c.parent().attr("data-id") + "&isenable=true", "",
              function (data) {
                  if (data.Code == App_G.Code.Code_200) {
                      App_G.MsgBox.success_digbox();
                      pager.execute(getSearchData());
    ...
    $("[name=disable]").digbox({
        ...
        CallBack: function (s, c, p) {
            ...
            $get("/SeoSetting/SetSeoKeyWordEnable?bid=" + c.parent().attr("data-id") + "&isenable=false", "",
              function (data) {
                  if (data.Code == App_G.Code.Code_200) {
                      App_G.MsgBox.success_digbox();
                      pager.execute(getSearchData());
